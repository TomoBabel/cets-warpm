...
def test_to_cets_from_project_root(project, tmp_path):
    ...
    assert sorted(order) == list(range(1, N + 1)) and all((doses[i] < doses[j]) == (order[i] < order[j]) for i in range(N) for j in range(N) if i != j)
...
def test_roundtrip_xml_fields_and_operators(project, tmp_path):
    ...
    for f in ("tilt_angle", "tilt_axis_angle", "tilt_axis_offset_x", "tilt_axis_offset_y", "dose", "defocus_um", "defocus_delta_um", "defocus_angle_deg", "phase_shift_pi"):
    ...
    assert settings.tomo_dims_px == [4096, 4096, 2000] and settings.exposure_per_tilt == 3.87 and settings.pixel_size_a == 1.54
    tomostar = WarpTomostar.from_file(str(back / "tomostar" / f"{STEM}.tomostar"))
    assert tomostar.n_rows == N and [r.angle_tilt for r in tomostar.rows] == pytest.approx([e.tilt_angle for e in src.entries])
...
def test_use_tilt_false_rows_and_constant_grid(tmp_path):
    def edit(xml: str) -> str:
        ...
        xml = re.sub(r"<GridMovementX.*?</GridMovementX>", '<GridMovementX Width="1" Height="1" Depth="1" MarginX="0" MarginY="0" MarginZ="0"><Node X="0" Y="0" Z="0" Value="7" /></GridMovementX>', xml, flags=re.S)
        xml = re.sub(r"<GridMovementY.*?</GridMovementY>", '<GridMovementY Width="1" Height="1" Depth="1" MarginX="0" MarginY="0" MarginZ="0"><Node X="0" Y="0" Z="0" Value="0" /></GridMovementY>', xml, flags=re.S)
        return xml
    ...
    assert [e.tilt_axis_offset_x for e in got.entries if e.use_tilt] == pytest.approx([e.tilt_axis_offset_x - 7.0 for e in src.entries if e.use_tilt])
...
def test_zero_dims_need_settings_and_source_enumeration(tmp_path):
    ...
    r = _run(["to-cets", str(xml), "-o", str(tmp_path / "a.cets.json"), "--drop-locals"])  # settings found next to the processing folder
    ...
    r = _run(["to-cets", str(xml), "-o", str(tmp_path / "c.cets.json"), "--drop-locals", "--volume-px", "4096x4096x2000", "--pix", "1.54"])
    ...
    (project / "proj.population").write_text('<?xml version="1.0"?><Population><Sources><Source GUID="x" Path="warp_tiltseries/proj.source"/></Sources></Population>')
    r = _run(["to-cets", str(project / "proj.population"), "-o", str(tmp_path / "d.cets.json"), "--drop-locals"])
    assert "Other.tomostar: listed in the .source" in r.stderr and "volume_dims_a = [6307.84, 6307.84, 3080.0]  [discovered]  (.source" in r.stdout
